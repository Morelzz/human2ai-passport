...
// --- LIVE (da completare quando avrai la chiave) ----------------------------
async function generateLive(input: HiggsfieldInput): Promise<HiggsfieldResult> {
  const apiKey = process.env.HIGGSFIELD_API_KEY;
  if (!apiKey) {
    throw new Error("HIGGSFIELD_API_KEY mancante: impossibile usare la modalità live");
  }
  if (!input.soulRef) {
    throw new Error("soulRef mancante: questo avatar non ha un modello Soul collegato");
  }

  // TODO (quando sarai online con la chiave): chiamare l'endpoint reale Higgsfield,
  // eventualmente fare polling sullo stato, e restituire l'URL finale.
  // La forma del risultato deve restare HiggsfieldResult.
  throw new Error("Modalità live non ancora configurata");
}
