      ...
      <section style={{ maxWidth: 760, margin: "0 auto", padding: "3rem 1.5rem" }}>
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            {items.map((r) => (
              <div key={r.id} style={{ background: "var(--surface)", border: "1px solid var(--hairline-soft)", borderRadius: 14, padding: "1.3rem" }}>
                ...
                {r.handle ? (
                  <Link href={`/passport/${r.handle}`} style={{ color: "var(--text)", fontWeight: 700, fontSize: "1rem", textDecoration: "none" }}>
                    @{r.handle}
                  </Link>
                ) : (
                  <span style={{ color: "var(--text-muted)", fontWeight: 700 }}>avatar rimosso</span>
                )}
                {r.certificate && (
                  <p style={{ color: "var(--text-faint)", fontSize: "0.72rem", fontFamily: "monospace", margin: "0.3rem 0 0", wordBreak: "break-all" }}>
                    cert: {r.certificate}
                  </p>
                )}

                {r.details && (
                  <p style={{ color: "var(--text-muted)", fontSize: "0.86rem", lineHeight: 1.5, margin: "0.7rem 0 0", background: "var(--bg)", border: "1px solid var(--hairline-soft)", borderRadius: 8, padding: "0.6rem 0.8rem" }}>
                    {r.details}
                  </p>
                )}
                {r.reporter_email && (
                  <p style={{ color: "var(--text-muted)", fontSize: "0.76rem", margin: "0.5rem 0 0" }}>Ricontatto: {r.reporter_email}</p>
                )}
                ...
                <input
                  ...
                  style={{ width: "100%", marginTop: "0.9rem", background: "var(--bg)", border: "1px solid var(--hairline)", borderRadius: 8, color: "var(--text)", fontSize: "0.82rem", padding: "0.55rem 0.7rem", outline: "none", boxSizing: "border-box" }}
                />

                <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.9rem", flexWrap: "wrap" }}>
                  <button onClick={() => decide(r.id, "dismiss")} disabled={busy === r.id}
                    style={{ padding: "0.55rem 1rem", borderRadius: 9, border: "1px solid var(--hairline)", background: "transparent", color: "var(--text-muted)", fontWeight: 700, fontSize: "0.82rem", cursor: busy === r.id ? "default" : "pointer" }}>
                    Archivia
                  </button>
                  <button onClick={() => decide(r.id, "uphold")} disabled={busy === r.id}
                    style={{ padding: "0.55rem 1rem", borderRadius: 9, border: "1px solid rgba(127,174,150,0.4)", background: "transparent", color: "var(--verified-c)", fontWeight: 700, fontSize: "0.82rem", cursor: busy === r.id ? "default" : "pointer" }}>
                    Accogli
                  </button>
                  <button onClick={() => decide(r.id, "suspend")} disabled={busy === r.id || !r.avatar_id}
                    ...
                    style={{ padding: "0.55rem 1rem", borderRadius: 9, border: "none", background: busy === r.id || !r.avatar_id ? "var(--elevated)" : "var(--blocked-c)", color: busy === r.id || !r.avatar_id ? "var(--text-muted)" : "var(--text)", fontWeight: 800, fontSize: "0.82rem", cursor: busy === r.id || !r.avatar_id ? "not-allowed" : "pointer" }}>
