...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [post, fonts, marchio] = await Promise.all([getPost(slug), geistOgFonts(), ogMarchio()]);
    ...
    (
      <OgCornice occhiello={`Il blog · ${post?.category ?? "Voci"}`} marchioUri={marchio}>
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          <div style={{ display: "flex", fontSize: lungo ? 58 : 68, letterSpacing: "-0.035em", lineHeight: 1.08, color: OG.crema, maxWidth: 1056 }}>
            {title}
          </div>
